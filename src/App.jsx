              ...
              <div key={e.id} style={{ display:"flex", alignItems:"flex-start", gap:8, marginBottom:9, opacity:ignored[e.id]?0.4:1 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4, flexShrink:0 }}>
                  ...
                  <div style={{ display:"flex", gap:4 }}>
                    <button
                      onClick={() => setIgnored(p => ({ ...p, [e.id]:!p[e.id] }))}
                      style={{ fontSize:9, padding:"2px 7px", borderRadius:4, border:`1px solid ${T.border}`, background:"transparent", color:ignored[e.id]?T.textMuted:T.textSub, cursor:"pointer", fontFamily:"inherit" }}
                    >{ignored[e.id] ? "unignore" : "ignore"}</button>
                    <button
                      onClick={() => setConfirmBlock(e)}
                      style={{ fontSize:9, padding:"2px 7px", borderRadius:4, border:`1px solid ${T.border}`, background:"transparent", color:T.urgent, cursor:"pointer", fontFamily:"inherit" }}
                      title="Never show this email again"
                    >block</button>
                  </div>
                </div>
              </div>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100, padding:24 }}>
          <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:14, maxWidth:400, width:"100%", padding:24 }}>
            ...
            <div style={{ fontSize:12, color:T.textMuted, marginBottom:20 }}>
              This email will never appear in your dashboard again. There is no way to undo this.
            </div>
            <div style={{ display:"flex", justifyContent:"flex-end", gap:8 }}>
              <GhostBtn onClick={() => setConfirmBlock(null)}>Cancel</GhostBtn>
              <PrimaryBtn
                color={T.urgent}
                onClick={() => {
